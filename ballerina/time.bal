...
public isolated function civilAddDuration(Civil civil, Duration duration) returns Civil|Error {
    ...
    decimal utcOffsetSeconds = utcOffset?.seconds ?: 0.0;
    decimal civilTimeSeconds = civil?.second ?: 0.0;
    int duYears = duration.years ?: 0;
    int duMonths = duration?.months ?: 0;
    int duDays = duration?.days ?: 0;
    int duHours = duration?.hours ?: 0;
    int duMinutes = duration?.minutes ?: 0;
    decimal duSeconds = duration?.seconds ?: 0.0;
    return externCivilAddDuration(civil.year, civil.month, civil.day, civil.hour, civil.minute, civilTimeSeconds,
            utcOffsetHours, utcOffsetMinutes, utcOffsetSeconds, timeAbbrev ?: "", zoneHandling, duYears, duMonths,
            duDays, duHours, duMinutes, duSeconds);
};
